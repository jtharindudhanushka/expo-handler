...
export async function POST(req: NextRequest) {
    try {
        ...
        const cleanRows = rows.map((r: Record<string, string>) => ({
            timestamp: parseCsvTimestamp(r.timestamp) || undefined,
        ...
        // Sort rows by parsed timestamp to ensure First-Come-First-Serve fairness
        const nowMs = Date.now();
        cleanRows.sort((a, b) => {
            const timeA = a.timestamp ? new Date(a.timestamp).getTime() : nowMs;
            const timeB = b.timestamp ? new Date(b.timestamp).getTime() : nowMs;
            return timeA - timeB;
        });
